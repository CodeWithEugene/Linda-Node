...
function FitAffectedAreas({ areas }: { areas: Area[] }) {
  const map = useMap()
  useEffect(() => {
    const points = areas.flatMap((area) => positionsFor(area.geometry).map(([lng, lat]) => [lat, lng] as [number, number]))
...
export function AreaMap({ areas, signals, onAreaSelect }: { areas: Area[]; signals: Signal[]; onAreaSelect?: (area: Area) => void }) {
  ...
  return <Box sx={{ position: 'relative', height: 300, borderRadius: 1, overflow: 'hidden', border: 1, borderColor: 'divider', bgcolor: '#dbeafe' }}>
    <MapContainer center={kenyaCenter} zoom={6} minZoom={4} maxZoom={14} scrollWheelZoom zoomControl style={{ height: '100%', width: '100%' }} aria-label="Interactive OpenStreetMap of affected areas">
      ...
      {affectedAreas.map(({ area, color, signalCount }) => <GeoJSONLayer key={area.id} data={area.geometry} style={{ color, weight: 2, fillColor: color, fillOpacity: .3 }} eventHandlers={{ click: () => onAreaSelect?.(area) }}><Popup><strong>{area.name}</strong><br />{signalCount} active signal{signalCount === 1 ? '' : 's'}<br />Click the polygon to prepare a case.</Popup></GeoJSONLayer>)}
    </MapContainer>
